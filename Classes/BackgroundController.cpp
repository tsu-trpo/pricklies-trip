...
BackgroundController::BackgroundController(Node &backgroundParent): startPoint{backgroundParent}
{
    ...
    for (int i = 0; i < 2; i++) {
        Background *background = Background::create(i + 1);
...
BackgroundController* BackgroundController::create(Node &backgroundParent)
{
    BackgroundController* background = new BackgroundController(backgroundParent);
